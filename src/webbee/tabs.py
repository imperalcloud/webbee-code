...
def _switch_handler(on_switch, idx: int):
    def _h(mouse_event):
...
def _close_handler(on_close, idx: int):
    def _h(mouse_event):
        from prompt_toolkit.mouse_events import MouseEventType
        if mouse_event.event_type == MouseEventType.MOUSE_UP:
            on_close(idx)
...
def tab_fragments(slots, *, on_switch, on_close, width: int = 0):
    ...
    frags.append((home_style, home_text, _switch_handler(on_switch, 0)))
    ...
    for idx, slot, prefix, suffix, close_text in pieces:
        ...
        frags.append((style, f"{prefix}{label}{suffix}", _switch_handler(on_switch, idx)))
        frags.append((TAB_STYLE_CLOSE, close_text, _close_handler(on_close, idx)))
